...
public class TicketService {
    ...
    public Long getTicketsSoldInLastMonth() {
        LocalDateTime startDate = LocalDateTime.now()
                .withDayOfMonth(1) // Ngày đầu tiên của tháng
                .withHour(0).withMinute(0).withSecond(0).withNano(0);
        LocalDateTime endDate = startDate.plusMonths(1);
        return ticketRepository.countTicketsSoldInLastMonth(startDate, endDate);
    }

    public Double getRevenueInLastMonth() {
        LocalDateTime startDate = LocalDateTime.now()
                .withDayOfMonth(1) // Ngày đầu tiên của tháng
                .withHour(0).withMinute(0).withSecond(0).withNano(0);
        LocalDateTime endDate = startDate.plusMonths(1);
        ...
        List<Ticket> tickets = ticketRepository.RevenueInLastMonth(startDate, endDate);
        double revenue = 0;
        if (tickets.size() == 0) {
            return 0.0;
        }
        for (Ticket ticket : tickets) {
            ...
            revenue = revenue + ticket.getFinalPrice();
        }
        return revenue;
    }
}
