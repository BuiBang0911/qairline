...
<head>
    ...
    <style>
        ...
        .custom-modal-content h6 {
            font-size: 22px; /* Lớn hơn và rõ ràng hơn */
            font-weight: 700; /* Chữ đậm */
            color: #34495e; /* Xanh đậm nhẹ */
            margin-bottom: 15px;
        ...
        .custom-modal-content .flight-price {
            font-size: 20px; /* Nổi bật giá */
        ...
        /* Thêm vị trí cố định cho các nút điều hướng */
        .custom-prev-button {
            position: absolute;
            left: 20px; /* Căn cách cạnh trái 20px */
            top: 50%; /* Căn giữa theo chiều dọc */
            transform: translateY(-50%); /* Dịch để nút nằm chính giữa */
        ...
        .custom-next-button {
            position: absolute;
            right: 20px; /* Căn cách cạnh phải 20px */
            top: 50%; /* Căn giữa theo chiều dọc */
            transform: translateY(-50%); /* Dịch để nút nằm chính giữa */
        ...
        /* Căn chỉnh nội dung modal để tránh xung đột */
        .custom-modal-content {
            position: relative; /* Đảm bảo các nút điều hướng nằm trong modal */
            padding: 40px 20px; /* Thêm khoảng trống để tránh nút */
        }
...
<script>
    ...
    function displayTickets(tickets, flightId) {
        ...
        let tmp = tickets;
        tmp = ticketFilter(tickets, flightId);
        console.log(tmp.size, typeof(tmp));
        console.log("tmp: ", tmp, "tickets: ", tickets)
        if (tmp.length === 0) {
            ...
            flightInfo.innerText = 'Flight ID: ' + tmp[currentTicketIndex].flightId;
            modalContent.appendChild(flightInfo);

            const seatNumber = document.createElement('p');
            seatNumber.innerText = 'seatNumber: ' + tmp[currentTicketIndex].seatNumber.toLocaleString();
            modalContent.appendChild(seatNumber);

            const status = document.createElement('p');
            status.innerText = 'status: ' + tmp[currentTicketIndex].status;
            modalContent.appendChild(status);

            const ticketPrice = document.createElement('p');
            ticketPrice.innerText = 'Price: ' + tmp[currentTicketIndex].finalPrice.toLocaleString() + ' VND';
            ...
            buyButton.setAttribute('onclick', 'buyTicket(' + tmp[currentTicketIndex].ticketId + ')');
            ...
            prevButton.onclick = function () {
                ...
                currentTicketIndex = (currentTicketIndex === 0) ? tmp.length - 1 : currentTicketIndex - 1;
                console.log(currentTicketIndex);
                displayTickets(tmp, flightId);
            };
            ...
            nextButton.onclick = function () {
                currentTicketIndex = (currentTicketIndex === tmp.length - 1) ? 0 : currentTicketIndex + 1;
                displayTickets(tmp, flightId);
            };
    ...
    function buyTicket(ticketId) {
        ...
        $(document).ready(function () {
            ...
            if (!userId) {
                ...
                // Thực hiện hành động nếu userId tồn tại
                console.log(`User ID found: ${userId}`);
                handleTicketPurchase(ticketId, userId);
            }
        });
    ...
    function handleTicketPurchase(ticketId, userId) {
        $.ajax({
            url: '/addtickets',
            type: 'POST',
